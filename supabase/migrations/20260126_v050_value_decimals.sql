...
-- Add value column (signed integer metric value; widened in later migration)
ALTER TABLE feedbacks ADD COLUMN IF NOT EXISTS value BIGINT DEFAULT 0;

-- Add value_decimals column (u8 decimal precision)
...
COMMENT ON COLUMN feedbacks.value IS 'Raw metric value (widened to i128 range in v0.6.0)';
